...
void loop()
{
  ...
  {
    delay(100);
         if (!digitalRead(HALL1PIN) || !digitalRead(HALL2PIN)) { sendHrData(heartRate1, HRMID); }    /// HRLOW
    else if (!digitalRead(HALL3PIN) || !digitalRead(HALL4PIN)) { sendHrData(heartRate1, HRHIGH); } // HRMID
    else if (!digitalRead(HALL5PIN) || !digitalRead(HALL6PIN))
    {
    ...
    if(hallSensorCount > 30) hallDone = true;
  }
